...
// Comprobamos de que haya elementos en la barra de ubicacion
if (document.getElementById("barraubi")){
    // En caso de que este vacia la barra de ubicacion la ocultamos
    if (document.getElementById("barraubi").childNodes.length == 1){
        document.getElementById("barraubi").style.display = "none";
    } // End if
} // End de la existencia de barra de ubicacion

/**
 * Estamos comprobando de que haya usuario registrado y que el numero de unidades del producto sea mayor que 0
 * @param {*} id ---> Le pasamos el id del producto
 * @param {*} unidades ---> Le pasamos las unidades del producto
 * @returns ---> Devolveremos un div y en ese div es posible que vaya un boton para añadir productos a la cesta o no
 */
function formulario(id, unidades){
    // Definimos nuestro div para añadir o no el boton
    let div = crearNodo("div", null, null, null, null);

    // Realizamos la conexion AJAX y hacemos por el metodo GET para comprobar de que haya o no usuario registrado
    $.ajax({
        type: "GET", // Indicaremos el tipo de metodo
        url: "/inicial/Usuario", // Hacemos la conexion a la accion de Usuario
        data: "", // Enviamos datos
        dataType: "json", // Como enviamos datos

        // En caso de que se hecho la conexion correcta comprobaremos si hay usuario o no registrado
        success: function (response) {
            // En caso de que haya usuario registrado y las unidades del producto sea mayor que 0 aparecerá el boton en el div
            if (response === true && unidades > 0){
                // Definimos el boton de anadir_producto y le asignamos algunos atributos
                let boton =  crearNodo("input", null, null, "btn btn-dark", "Añadir Producto");
                boton.setAttribute("name", "anadir_producto");
                boton.setAttribute("type", "button");

                // Le asignamos al boton el evento onclick para que añada elementos a la cesta
                boton.onclick =  function(){
                    $.ajax({
                        type: "POST", // Indicaremos el tipo de metodo
                        url: "/cataproducto/AnadirProductos", // Hacemos la conexion a la accion de Usuario
                        data: {cod_producto : id}, // Enviamos datos
                        dataType: "json", // Como enviamos datos
                        success: function (response) {
